...
We implemented DP-GCD, DP-SGD (batch size 1 with Gaussian noise), and DP-CD (randomized coordinate descent with Gaussian noise) in Python and evaluated them on representative datasets, focusing on the most interesting implementations from a methodological standpoint.

\subsection*{Experimental Setup}

\paragraph{Key Datasets.}
\begin{itemize}
\item \textbf{log2:} $n = 1000$, $p = 100$, quasi-sparse weights via $w_{\text{true}} \sim \text{LogNormal}(0, \sigma=2.0)$, logistic regression. \textit{Showcases automatic adaptation to quasi-sparsity.}

\item \textbf{square:} $n = 1000$, $p = 1000$, exactly $s = 10$ non-zero entries in $w_{\text{true}}$, linear regression. \textit{Tests high-dimensional sparse regime exploiting sparsity directly.}

\item \textbf{dorothea-like:} $n = 800$, $p = 5000$, $s = 50$ quasi-sparse, logistic classification. \textit{Extreme high-dimensional case testing robustness and scalability.}
\end{itemize}

\paragraph{Privacy budget.} We used $\varepsilon = 1$, $\delta = 1/n^2$ across all experiments. Noise scales were tuned empirically for convergence visualization (qualitative reproduction rather than strict theoretical calibration), chosen to balance utility with computational stability.

\paragraph{Metric.} We compute a non-private optimum $w^*$ via full gradient descent and plot the relative error
\[
\frac{f(w_t) - f(w^*)}{f(w^*)}
\]
versus \emph{passes on data}, where 1 pass = $n$ steps (DP-SGD), $p$ steps (DP-CD), or 1 step (DP-GCD). This normalization ensures fair comparison across algorithms with different per-iteration costs.
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{log2_results.pdf}
    \caption{log2 (quasi-sparse logistic)}
    \label{fig:log2}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{square_results.pdf}
    \caption{square (sparse linear)}
    \label{fig:square}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{dorothea_results.pdf}
    \caption{Dorothea-like (high-dimensional logistic)}
    \label{fig:dorothea}
  \end{subfigure}
  \caption{Relative error vs.\ passes on data for three representative experiments. In all cases, DP-GCD converges faster and/or to a lower error than DP-SGD and DP-CD, with the strongest gains in the high-dimensional sparse regime.}
  \label{fig:combined}
\end{figure}


On \textbf{log2}, DP-GCD exploits quasi-sparsity from heavy-tailed log-normal weights and reaches a relative error around $10^{-3}$ within tens of passes, while DP-SGD and DP-CD plateau above $10^{-2}$. On the \textbf{square} experiment, with $p=1000$ and only 10 non-zero coordinates, DP-GCD attains roughly an order-of-magnitude lower error than the baselines under the same privacy budget. In the extreme \textbf{dorothea-like} setting ($p = 5000$, $s=50$, $n=800$), DP-GCD is effectively the only method that substantially decreases the objective, whereas DP-SGD and DP-CD fail to make progress, illustrating the benefit of logarithmic dependence on $p$ in truly high-dimensional sparse problems.

\section{Implementation Insights}

\subsection{Key Implementation Details}

\textbf{Report-Noisy-Max in Practice.} The crucial step in DP-GCD is the private greedy selection:
\[
j_t = \arg\max_{j \in [p]} \left\{ |g_{t,j}|/\sqrt{M_j} + \mathrm{Lap}(\lambda_j) \right\}.
\]
In implementation, we compute the true gradient $g_t = \nabla f(w_t)$ (not privatized), then add coordinate-wise Laplace noise before taking the argmax. This is efficient ($O(p)$ per iteration) and does not require sorting or complex data structures.

\textbf{Noise Calibration.} A key insight is that different coordinates $j$ can have different Lipschitz constants $L_j$. For smooth losses (e.g., logistic regression), $L_j$ is bounded by a loss-dependent constant that can be computed from the data geometry. Using coordinate-wise noise scales allows DP-GCD to concentrate privacy budget on informative coordinates, further amplifying its advantage in sparse regimes.

\textbf{Convergence Behavior.} In practice, DP-GCD exhibits the predicted convergence pattern: rapid initial progress (exponentially decaying error) that eventually plateaus due to accumulated privacy noise. The plateau depends on the total privacy budget and the problem conditioning. With our empirically tuned noise, we observe convergence within 50--60 passes for well-conditioned instances.

\section{Discussion and Conclusion}

\paragraph{Strengths.}
DP-GCD is an unconstrained method that achieves logarithmic dependence on the ambient dimension in regimes where quasi-sparsity and coordinate-wise regularity hold. It provides near-optimal rates in the strongly convex setting, adapts automatically to effective sparsity without requiring explicit $\ell_1$ constraints or prior knowledge of the support, and is empirically competitive on high-dimensional benchmarks. The implementation is straightforward and does not require specialized data structures.
...
\paragraph{Takeaway.}
Our work confirms the core message of Mangold et al.: by combining the exponential mechanism (Report-Noisy-Max) with greedy coordinate descent, one can achieve dramatic improvements in high-dimensional private learning when quasi-sparse structure is present. The implementation is clean and the experimental results are compelling, particularly on the extreme high-dimensional case where DP-GCD is the only viable baseline. In applications where high dimensionality and approximate sparsity coexist—such as genomics, NLP, and high-dimensional regression—DP-GCD offers a principled and practical approach.
