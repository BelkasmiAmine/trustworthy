...
\textbf{Exponential Mechanism (Course Slide 35):}
\begin{itemize}
...
\vspace{0.8em}
\textbf{Report-Noisy-Max as Special Case (Course Slide 36):}
\begin{itemize}
...
\item \textbf{Sensitivity} (Course Slides 28-29): Coordinate $j$ gradient has $\ell_1$-sensitivity $\Delta_1^j = 2L_j/n$
\item \textbf{Laplace mechanism} (Course Slide 29): Adding $\text{Lap}(\Delta_1^j/\varepsilon_t)$ gives $\varepsilon_t$-DP
\item \textbf{Advanced composition} (Course Slide 26, Theorem 3.16): Over $2T$ mechanisms with $\varepsilon_t = \varepsilon/(2\sqrt{2T\log(1/\delta)})$ yields $(\varepsilon, \delta)$-DP
\end{enumerate}
...
% \vspace{0.5em}
% \textbf{Course connection:} Beating worst-case lower bounds through structure exploitation---key theme from privacy-utility lecture
\end{frame}
...
\section{Connection to Course}

\begin{frame}{Links to Course Concepts (1/2)}
\textbf{Differential Privacy Fundamentals}
\begin{itemize}
\item \textbf{Laplace Mechanism} (Slides 28-30): Scale $\Delta_1/\varepsilon$ for $\ell_1$-sensitivity $\Delta_1$
\item \textbf{Exponential Mechanism} (Slides 34-38): Utility-based sampling, Report-Noisy-Max as special case
\item \textbf{Composition} (Slide 26): Basic composition adds privacy parameters
\item \textbf{Advanced Composition} (Slide 71-72, Moments Accountant): $\sqrt{k}$ dependence for $k$ mechanisms
\item \textbf{Sensitivity Analysis} (Slides 28-29): Coordinate-wise $\ell_1$-sensitivity enables fine-grained noise
\end{itemize}

\vspace{0.5em}
\textbf{Privacy-Utility Trade-off}
\begin{itemize}
\item \textbf{Tradeoff Curves} (Slides 11, 21): Type I/II error bounds for statistical tests
\item Lower bounds show polynomial $p$ dependence in worst-case
\item DP-GCD exploits structure to beat worst-case via $\ell_1$-norm quantities
\item Sample complexity: $n^{-2}$ rate matches non-private optimal (up to log factors)
\end{itemize}
\end{frame}

\begin{frame}{Links to Course Concepts (2/2)}
\textbf{Differentially Private Learning}
\begin{itemize}
\item \textbf{DP-ERM Framework} (Slide 58): Regularized empirical risk minimization
\item \textbf{Output Perturbation} (Slides 61-65): Perturb optimizer directly (requires strong convexity)
\item \textbf{Gradient Perturbation} (Slide 66, DP-GD): Privatize gradients during optimization
\item \textbf{DP-SGD} (Slides 67-73): Mini-batch gradient + clipping + Gaussian noise
\item \textbf{Privacy Amplification by Subsampling} (Slides 39-41): Noise reduction by factor $\sqrt{q}$ where $q = m/n$
\item \textbf{Moments Accountant / RDP} (Slides 51-55, 70-73): Tighter composition tracking for Gaussian mechanisms
\end{itemize}

\vspace{0.5em}
\textbf{High-Dimensional Statistics \& Optimization}
\begin{itemize}
\item Sparsity assumptions: effective dimension $s \ll p$
\item $\ell_1$ vs $\ell_2$ norm characterizations (DP-GCD uses $\ell_1$-based bounds)
\item Convexity, smoothness, strong convexity (Slides 62-63)
\item Coordinate descent as alternative to full gradient methods
\end{itemize}
\end{frame}
...
\vspace{0.5em}
\textbf{Classical Advanced Composition} (Theorem 3.20 in Dwork \& Roth, implicit in course):
\[
\varepsilon = \sqrt{2k\log(1/\delta)} \cdot \varepsilon_0 + k\varepsilon_0^2
\]

\vspace{0.5em}
\textbf{Renyi Differential Privacy} (Course Slides 51-55):
\begin{itemize}
...
\item \textbf{Composition} (Slide 53): $(\alpha, \rho_1)$-RDP + $(\alpha, \rho_2)$-RDP $\Rightarrow$ $(\alpha, \rho_1 + \rho_2)$-RDP
\item \textbf{Conversion} (Slide 53): $(\alpha, \rho)$-RDP $\Rightarrow$ $(\rho + \log(1/\delta)/(\alpha-1), \delta)$-DP
\end{itemize}
